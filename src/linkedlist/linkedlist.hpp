...
class LinkedList final {
 public:
  ...
  void insert_at_tail(T data) noexcept {
  ...
  [[nodiscard]] bool equal(LinkedList<T>* rhs) const noexcept {
  ...
  [[nodiscard]] bool delete_data(T data) {
    if (is_empty()) {
      return false;
    }
    ...
    auto current_node = head_;
    while (current_node->get_next_node() != nullptr &&
           current_node->get_next_node()->get_node_data() != data) {
      current_node = current_node->get_next_node();
    }
  ...
  [[nodiscard]] NodePtr get_head() const noexcept { return head_; }

  [[nodiscard]] NodePtr get_tail(NodePtr current_node) const noexcept {
    if (current_node->get_next_node() != nullptr) {
      current_node = current_node->get_next_node();
...
return 0;
 ...
 private:
  ...
  std::shared_ptr<Node<T>> end_{};  // end of the list TODO insert at tail O(1) tail updates everytime a new node is added
