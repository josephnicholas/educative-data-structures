...
struct TrieNode {
    explicit TrieNode() : is_end_of_node_{false} {
    ...
    std::array<TrieNode *, ALPHABET_LENGTH> children_{}; // array of pointers to children nodes
...
struct Trie final : TrieNode {
    ...
    void get_words_and_store(TrieNode *root_node, std::vector<std::string> &result,
        ...
        for (int idx = 0; idx < ALPHABET_LENGTH; idx++) {
            if (root_node->children_.at(idx) != nullptr) {
                word[level] = static_cast<char>('a' + idx);
                get_words_and_store(root_node->children_.at(idx), result, level + 1, word, keyword);
            }
        }
    }
    ...
    bool node_has_no_children(TrieNode *current_node) const noexcept {
        for (int idx = 0; idx < ALPHABET_LENGTH; idx++) {
            if (current_node->children_.at(idx) != nullptr) {
                return false;
            }
        }
